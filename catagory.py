...
class categoryClass:
    def __init__(self,root,):
        ...
        lbl_title=Label(self.root,text="Manage Product Catagory",font=("goudy old style",30,"bold"),bg="#184a45",fg="white",bd=3,relief=RIDGE).pack(side=TOP,fill=X,padx=10,pady=20)

        lbl_name=Label(self.root,text="Enter Catagory Name",font=("goudy old style",30,"bold"),bg="white").place(x=50,y=100)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category already assigend, try different",parent=self.root)
                else:
                    cur.execute("Insert into category (name) values(?)",(self.var_name.get(),))
                    con.commit()
                    messagebox.showinfo("Success","Category Addedd Scuccessfully",parent=self.root)
                    self.show()                                                                     
