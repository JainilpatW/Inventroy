...
class employeeClass:
    ...
    #Combobox
        ...
        cmd_serach=ttk.Combobox(searchFrame,textvariable=self.var_searchby,values=("Select","Email","Name","Contact"),state='readonly',justify=CENTER,font=("goudy old  style",15))
        cmd_serach.place(x=10,y=10,width=180)
        cmd_serach.current(0)

        txt_serch=Entry(searchFrame,textvariable=self.var_searchtxt,font=("goudy old  style",15),bg="lightyellow").place(x=200,y=10)
        btn_serch=Button(searchFrame,text="search",command=self.search,font=("goudy old  style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=430,y=9,width=150,height=30)
    ...
    #btn
        ...
        btn_update=Button(self.root,text="UPDATE",command=self.update,font=("goudy old  style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=620,y=305,width=110,height=30)
        btn_delete=Button(self.root,text="DELETE",command=self.delete,font=("goudy old  style",15),bg="#f44336",fg="white",cursor="hand2").place(x=740,y=305,width=110,height=30)
        btn_clear=Button(self.root,text="CLEAR",command=self.clear,font=("goudy old  style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=860,y=305,width=110,height=30)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Emoplyee Id already as define",parent=self.root)
                else:
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Employee Addedd Scuccessfully",parent=self.root)
                    self.show()                                                                     
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row==None:
                    ...
                    con.commit()
                    messagebox.showinfo("Success","Employee Update Scuccessfully",parent=self.root)
                    self.show()                                                                     
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row==None:
                    ...
                    op=messagebox.askyesno("Confrom","Confrom Detele?",parent=self.root)
                    if op==True:
    ...
    def search(self):
        ...
        try:
            if self.var_searchby.get()=="Select":
                messagebox.showerror("Error","Select Option",parent=self.root)
